...
Action.HealTheSick = new MultipartAction("Heal The Sick", {
    ...
    tickProgress(offset) {
        return getSkillLevel("Magic") * Math.min(getSkillLevel("Restoration"), 1) * (1 + getLevel(this.loopStats[(towns[0].HealLoopCounter + offset) % this.loopStats.length]) / 100) * Math.sqrt(1 + towns[0].totalHeal / 100);
    },
    loopsFinished() {
