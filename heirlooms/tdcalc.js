...
function startTDCalc() {
  ...
  function calcFire(c, shocked) {
    var thisFireDamage = traps.fire.damage * lightColMult(c);
    var thisAddDamage = thisFireDamage;
    if (detailed[c].shocked) thisAddDamage = thisFireDamage * traps.lightning.damageBuff;
    ...
    if (strengthLocations[detailed[c].row]) thisAddDamage *= traps.strength.effect;
    if (detailed[c].chilled && traps.frost.level >= 3) thisAddDamage *= traps.frost.fireIncrease;
    return thisAddDamage;
  }
  
  function calcFrost(c) {
    return (detailed[c].shocked ? traps.frost.damage * traps.lightning.damageBuff : traps.frost.damage);
  }
  ...
  function calcPoison(c, shocked, hp, dmg) {
    ...
    if (detailed[c].shocked) {
      output.stack *= traps.lightning.damageBuff;
    }
  ...
  function calcLightning(c) {
    var thisAddDamage = traps.lightning.damage;
    if (detailed[c].shocked) thisAddDamage = traps.lightning.damage * traps.lightning.damageBuff;
    if (detailed[c].chilled || detailed[c].frozen) thisAddDamage += traps.lightning.damage * traps.lightning.damageBuff;
    if (detailed[c].frozen) thisAddDamage += traps.lightning.damage * traps.lightning.damageBuff;
    return thisAddDamage;
  }
  ...
  function calcStrength(c, shocked) {
    ...
    if (detailed[c].shocked) thisAddDamage = strengthDamage * traps.lightning.damageBuff;
  ...
  function getStrengthDamage(data) {
    ...
    var amountOfFire = 0;
    var returnDamage = traps.fire.damage * traps.strength.effect;
  ...
  function estimatedMaxDifficulty(maxHealth) {
    ...
    //enemies = gotRows;
    //avgRS = 60 / ticks * enemies; // how many times do people die in a min;
    //avgRS *= Math.ceil(getHealthWith(difficulty) / 20); // how much RS
    // $("#averageRS").text(prettify(avgRS));
  }
  ...
  function getLightningMultiplier(length, times, type) {
    ...
    if (length == 0 || length < 0) {
      ...
      if (type == "condenser") {
        ...
        return traps.lightning.damageBuff;
      }
    }
  }
  
  function lightColMult(cell) {
    return traps.lightning.level >= 4 ? (1 + 0.1 * lightColStacks[cell % 5]) : 1;
  }
