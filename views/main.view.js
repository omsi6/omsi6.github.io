...
function View() {
	...
	this.requestUpdate = function (category, target) {
		if (!this.requests[category].includes(target)) {
			this.requests[category].push(target);
		}
	};
	...
	this.update = function () {
		...
		if (dungeonShowing !== undefined) {
			this.updateSoulstoneChance(dungeonShowing);
		}
	...
	this.adjustTooltipPosition = function (tooltipDiv) {
		...
		let tooltipX = tooltipDiv.getBoundingClientRect().x + tooltipDiv.clientWidth;
		if (tooltipX > windowWidth/3) {
			tooltipDiv.classList.add('showthisR');
			tooltipDiv.classList.remove('showthisL');
		} else {
			tooltipDiv.classList.add('showthisL');
			tooltipDiv.classList.remove('showthisR');
		}
	}
	...
	this.updateStat = function (stat) {
		...
		document.getElementById(`stat${stat}Level`).textContent     = intToString(level,
																				  1);
		document.getElementById(`stat${stat}Talent`).textContent    = intToString(talent,
																				  1);
		
		if (statShowing === stat || document.getElementById(`stat${stat}LevelExp`).innerHTML === "") {
			document.getElementById(`stat${stat}Level2`).textContent         = intToString(level,
																						   1);
			const expOfLevel                                                 = getExpOfLevel(level);
			document.getElementById(`stat${stat}LevelExp`).textContent       = intToString(stats[stat].exp - expOfLevel,
																						   1);
			document.getElementById(`stat${stat}LevelExpNeeded`).textContent = intToString(`${getExpOfLevel(level + 1) - expOfLevel}`,
																						   1);
			document.getElementById(`stat${stat}LevelProgress`).textContent  = intToString(levelPrc,
																						   2);
			
			document.getElementById(`stat${stat}Talent2`).textContent         = intToString(talent,
																							1);
			const expOfTalent                                                 = getExpOfTalent(talent);
			document.getElementById(`stat${stat}TalentExp`).textContent       = intToString(stats[stat].talent - expOfTalent,
																							1);
			document.getElementById(`stat${stat}TalentExpNeeded`).textContent = intToString(`${getExpOfTalent(talent + 1) - expOfTalent}`,
																							1);
			document.getElementById(`stat${stat}TalentMult`).textContent      = intToString(calcTalentMult(talent),
																							3);
			document.getElementById(`stat${stat}TalentProgress`).textContent  = intToString(talentPrc,
																							2);
			document.getElementById(`stat${stat}TotalMult`).textContent       = intToString(getTotalBonusXP(stat),
																							3);
		}
	};
	...
	this.updateSkill = function (skill) {
		...
		if (skillShowing === skill) {
			...
			document.getElementById(`skill${skill}LevelExp`).textContent       = intToString(skills[skill].exp - expOfLevel,
																							 1);
			document.getElementById(`skill${skill}LevelExpNeeded`).textContent = intToString(`${getExpOfSkillLevel(getSkillLevel(skill) + 1) - expOfLevel}`,
																							 1);
			document.getElementById(`skill${skill}LevelProgress`).textContent  = intToString(levelPrc,
																							 2);
			
			if (skill === "Dark") {
				document.getElementById("skillBonusDark").textContent = intToString(getSkillBonus("Dark"),
																					4);
			} else if (skill === "Chronomancy") {
				document.getElementById("skillBonusChronomancy").textContent = intToString(getSkillBonus("Chronomancy"),
																						   4);
			} else if (skill === "Practical") {
				document.getElementById("skillBonusPractical").textContent = getSkillBonus("Practical").toFixed(3).replace(/(\.\d*?[1-9])0+$/gu,
																														   "$1");
			} else if (skill === "Mercantilism") {
				document.getElementById("skillBonusMercantilism").textContent = intToString(getSkillBonus("Mercantilism"),
																							4);
			} else if (skill === "Spatiomancy") {
				document.getElementById("skillBonusSpatiomancy").textContent = getSkillBonus("Spatiomancy").toFixed(3).replace(/(\.\d*?[1-9])0+$/gu,
																															   "$1");
			} else if (skill === "Divine") {
				document.getElementById("skillBonusDivine").textContent = intToString(getSkillBonus("Divine"),
																					  4);
			} else if (skill === "Commune") {
				document.getElementById("skillBonusCommune").textContent = getSkillBonus("Commune").toFixed(3).replace(/(\.\d*?[1-9])0+$/gu,
																													   "$1");
			} else if (skill === "Wunderkind") {
				document.getElementById("skillBonusWunderkind").textContent = intToString(getSkillBonus("Wunderkind"),
																						  4);
			} else if (skill === "Gluttony") {
				document.getElementById("skillBonusGluttony").textContent = getSkillBonus("Gluttony").toFixed(3).replace(/(\.\d*?[1-9])0+$/gu,
																														 "$1");
			} else if (skill === "Thievery") {
				document.getElementById("skillBonusThievery").textContent = intToString(getSkillBonus("Thievery"),
																						4);
			} else if (skill === "Leadership") {
				document.getElementById("skillBonusLeadership").textContent = intToString(getSkillBonus("Leadership"),
																						  4);
			}
		}
	...
	this.updateTime           = function () {
		...
		document.getElementById("timer").textContent   = `${intToString((timeNeeded - timer),
																		1)} | ${formatTime((timeNeeded - timer) / 50 / getActualGameSpeed())}`;
	};
	...
	this.updateResource       = function (resource) {
		if (resource !== "gold") {
			document.getElementById(`${resource}Div`).style.display = resources[resource] ? "inline-block" : "none";
		}
	...
	this.updateTeamCombat     = function () {
		if (towns[2].unlocked) {
			...
			document.getElementById("skillSCombatLevel").textContent       = intToString(getSelfCombat(),
																						 1);
			document.getElementById("skillTCombatLevel").textContent       = intToString(getTeamCombat(),
																						 1);
		} else {
	...
	this.updateNextActions    = function () {
		...
		for (let i = 0; i < actions.next.length; i++) {
			...
			for (const collapse of collapses) {
				if (townNum === collapse.zone && i < collapse.index) {
	...
	this.updateCurrentActionBar = function (index) {
		...
		if (action.errorMessage) {
			...
			if (action.name === "Heal The Sick") {
				unlockStory("failedHeal");
			}
			if (action.name === "Brew Potions" && resources.reputation >= 0 && resources.herbs >= 10) {
				unlockStory("failedBrewPotions");
			}
			if (action.name === "Brew Potions" && resources.reputation < 0 && resources.herbs >= 10) {
				unlockStory("failedBrewPotionsNegativeRep");
			}
			if (action.name === "Gamble" && resources.reputation < -5) {
				unlockStory("failedGamble");
			}
			if (action.name === "Gamble" && resources.gold < 20 && resources.reputation > -6) {
				unlockStory("failedGambleLowMoney");
			}
			if (action.name === "Gather Team") {
				unlockStory("failedGatherTeam");
			}
			if (action.name === "Craft Armor") {
				unlockStory("failedCraftArmor");
			}
		...
		if (curActionShowing === index) {
			...
			for (const stat of statList) {
				if (action[`statExp${stat}`]) {
					statExpGain += `<div class='bold'>${_txt(`stats>${stat}>short_form`)}:</div> ${intToString(action[`statExp${stat}`],
																											   2)}<br>`;
				}
			}
	...
	this.mouseoverAction = function (index, isShowing) {
		if (isShowing) {
			curActionShowing = index;
		} else {
			curActionShowing = undefined;
		}
	...
	this.updateProgressAction = function (varName, town) {
		...
		document.getElementById(`progress${varName}`).textContent = intToString(levelPrc,
																				2);
	...
	this.updateProgressActions = function () {
		for (const town of towns) {
			for (let i = 0; i < town.progressVars.length; i++) {
				...
				this.updateProgressAction(varName,
										  town);
			}
		}
	};
	...
	this.updateLockedHidden = function () {
		for (const action of totalActionList) {
			...
			if (action.allowed && getNumOnList(action.name) >= action.allowed()) {
				addClassToDiv(actionDiv,
							  "capped");
			} else if (action.unlocked()) {
				if (infoDiv) {
					removeClassFromDiv(infoDiv,
									   "hidden");
				}
				removeClassFromDiv(actionDiv,
								   "locked");
				removeClassFromDiv(actionDiv,
								   "capped");
			} else {
		...
		for (const action of totalActionList) {
			if (action.storyReqs !== undefined) {
				...
				if (init || document.getElementById(divName).innerHTML.includes("???")) {
					...
					const name           = action.name.toLowerCase().replace(/ /gu,
																			 "_");
					const storyAmt       = _txt(`actions>${name}`,
												"fallback").split("⮀").length - 1;
					let storiesUnlocked  = 0;
					for (let i = 1; i <= storyAmt; i++) {
						const storyText = _txt(`actions>${name}>story_${i}`,
											   "fallback").split("⮀");
	...
	this.showTown = function (townNum) {
		...
		if (actionStoriesShowing) {
			actionStoriesTown[townNum].style.display = "block";
		} else {
	...
	this.updateAddAmount = function (num) {
		...
		if (num > 0) {
			removeClassFromDiv(document.getElementById(`amount${num}`),
							   "unused");
		}
	};
	...
	this.updateLoadout = function (num) {
		...
		if (elem) {
			removeClassFromDiv(document.getElementById(`load${num}`),
							   "unused");
		}
	};
	...
	this.createTownActions = function () {
		if (actionOptionsTown[0].firstChild) {
			return;
		}
		for (const prop in Action) {
			...
			if (action.type === "limited") {
				this.createTownInfo(action);
			}
			if (action.type === "progress") {
				this.createActionProgress(action);
			}
			if (action.type === "multipart") {
				this.createMultiPartPBar(action);
			}
		}
	};
	...
	this.createTownAction = function (action) {
		...
		if (action.storyReqs !== undefined) {
			let storyTooltipText = "";
			let lastInBranch     = false;
			const storyAmt       = _txt(`actions>${action.name.toLowerCase().replace(/ /gu,
																					 "_")}`,
										"fallback").split("⮀").length - 1;
			for (let i = 1; i <= storyAmt; i++) {
				if (_txt(`actions>${action.name.toLowerCase().replace(/ /gu,
																	  "_")}>story_${i}`) === undefined) {
					console.log(`actions>${action.name.toLowerCase().replace(/ /gu,
																			 "_")}>story_${i}`);
				}
				const storyText = _txt(`actions>${action.name.toLowerCase().replace(/ /gu,
																					"_")}>story_${i}`,
									   "fallback").split("⮀");
				if (action.storyReqs(i)) {
					storyTooltipText += storyText[0] + storyText[1];
	...
	this.adjustGoldCosts = function () {
		for (const action of actionsWithGoldCost) {
			this.adjustGoldCost(action.varName,
								action.goldCost());
		}
	};
	...
	this.showDungeon = function (index) {
		...
		if (index !== undefined) {
			this.updateSoulstoneChance(index);
		}
	};
	...
	this.updateSoulstoneChance = function (index) {
		...
		for (let i = 0; i < dungeon.length; i++) {
			...
			document.getElementById(`soulstoneChance${index}_${i}`).textContent    = intToString(level.ssChance * 100,
																								 4);
	...
	this.updateSoulstones = function () {
		for (const stat of statList) {
			if (stats[stat].soulstone) {
				...
				document.getElementById(`ss${stat}`).textContent            = intToString(stats[stat].soulstone,
																						  1);
				...
				document.getElementById(`stat${stat}ss`).textContent        = intToString(stats[stat].soulstone,
																						  1);
			} else {
	...
	this.changeTheme = function (init) {
		if (init) {
			document.getElementById("themeInput").value = options.theme;
		}
	...
	this.adjustDarkRitualText = function () {
		...
		if (getBuffLevel("Ritual") > 200) {
			DRdesc.innerHTML += DarkRitualDescription[9];
		}
	}
}
...
function adjustActionListSize(amt) {
	if (document.getElementById("expandableList").style.height === "" && amt > 0) {
		...
		document.getElementById("expandableList").style.height = `${Math.min(Math.max(parseInt(document.getElementById("expandableList").style.height) + amt,
																					  500),
																			 2000)}px`;
		curActionsDiv.style.maxHeight                          = `${Math.min(Math.max(parseInt(curActionsDiv.style.maxHeight) + amt,
																					  457),
																			 1957)}px`;
		nextActionsDiv.style.maxHeight                         = `${Math.min(Math.max(parseInt(nextActionsDiv.style.maxHeight) + amt,
																					  457),
																			 1957)}px`;
	}
	setScreenSize();
...
function updateBuffCaps() {
	for (const buff of buffList) {
		document.getElementById(`buff${buff}Cap`).value = Math.min(parseInt(document.getElementById(`buff${buff}Cap`).value),
																   buffHardCaps[buff]);
...
function clampDivs() {
	...
	tooltips.forEach(tooltip => {
		...
		if (offsets.top != 0) {
			console.log("Top: " + offsets.top + ". Height: " + tooltip.clientHeight + ". Total: " + (offsets.top + tooltip.clientHeight) + ". Screensize: " + screenSize);
		}
		if (offsets.top < 0) {
			console.log("Offscreen - top");
		}
		if (offsets.top + tooltip.clientHeight > screenSize) {
